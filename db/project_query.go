...
func (pq *ProjectQuery) QueryCustomer() *CustomerQuery {
...
func (pq *ProjectQuery) QuerySurveyInvitations() *SurveyInvitationQuery {
	query := (&SurveyInvitationClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withCustomer:          pq.withCustomer.Clone(),
		withSurveyInvitations: pq.withSurveyInvitations.Clone(),
...
func (pq *ProjectQuery) WithCustomer(opts ...func(*CustomerQuery)) *ProjectQuery {
	query := (&CustomerClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) WithSurveyInvitations(opts ...func(*SurveyInvitationQuery)) *ProjectQuery {
	query := (&SurveyInvitationClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withSurveyInvitations = query
	return pq
}
...
//		GroupBy(project.FieldProspectiveID).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.Project.Query().
//		Select(project.FieldProspectiveID).
//		Scan(ctx, &v)
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	var (
		...
		loadedTypes = [2]bool{
			pq.withCustomer != nil,
			pq.withSurveyInvitations != nil,
		}
	)
	if pq.withCustomer != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withCustomer; query != nil {
		if err := pq.loadCustomer(ctx, query, nodes, nil,
			func(n *Project, e *Customer) { n.Edges.Customer = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withSurveyInvitations; query != nil {
		if err := pq.loadSurveyInvitations(ctx, query, nodes,
			func(n *Project) { n.Edges.SurveyInvitations = []*SurveyInvitation{} },
			func(n *Project, e *SurveyInvitation) {
...
func (pq *ProjectQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Project, init func(*Project), assign func(*Project, *Customer)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Project)
	for i := range nodes {
		if nodes[i].customer_projects == nil {
			continue
		}
		fk := *nodes[i].customer_projects
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "customer_projects" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *ProjectQuery) loadSurveyInvitations(ctx context.Context, query *SurveyInvitationQuery, nodes []*Project, init func(*Project), assign func(*Project, *SurveyInvitation)) error {
	...
	query.Where(predicate.SurveyInvitation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.SurveyInvitationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_survey_invitations
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_survey_invitations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_survey_invitations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func (cq *CustomerQuery) QuerySurveyInvitations() *SurveyInvitationQuery {
	query := (&SurveyInvitationClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) QueryProjects() *ProjectQuery {
...
func (cq *CustomerQuery) Clone() *CustomerQuery {
	...
	return &CustomerQuery{
		...
		withSurveyInvitations: cq.withSurveyInvitations.Clone(),
...
func (cq *CustomerQuery) WithSurveyInvitations(opts ...func(*SurveyInvitationQuery)) *CustomerQuery {
	query := (&SurveyInvitationClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withSurveyInvitations = query
	return cq
}

// WithProjects tells the query-builder to eager-load the nodes that are connected to
// the "projects" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CustomerQuery) WithProjects(opts ...func(*ProjectQuery)) *CustomerQuery {
	query := (&ProjectClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(customer.FieldProspectiveID).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.Customer.Query().
//		Select(customer.FieldProspectiveID).
//		Scan(ctx, &v)
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withSurveyInvitations; query != nil {
		if err := cq.loadSurveyInvitations(ctx, query, nodes,
			func(n *Customer) { n.Edges.SurveyInvitations = []*SurveyInvitation{} },
			func(n *Customer, e *SurveyInvitation) {
				n.Edges.SurveyInvitations = append(n.Edges.SurveyInvitations, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := cq.withProjects; query != nil {
		if err := cq.loadProjects(ctx, query, nodes,
			func(n *Customer) { n.Edges.Projects = []*Project{} },
			func(n *Customer, e *Project) { n.Edges.Projects = append(n.Edges.Projects, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CustomerQuery) loadSurveyInvitations(ctx context.Context, query *SurveyInvitationQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *SurveyInvitation)) error {
	...
	query.Where(predicate.SurveyInvitation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.SurveyInvitationsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.customer_survey_invitations
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_survey_invitations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_survey_invitations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CustomerQuery) loadProjects(ctx context.Context, query *ProjectQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Project)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Customer)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Project(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.ProjectsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.customer_projects
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_projects" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_projects" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
